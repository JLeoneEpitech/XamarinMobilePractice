...
public class ApiClient : IDisposable
{
    ...
    public async Task<List<T>> GetAsync<T>(string url)
    {
        try
        {
            ...
            var opts = new JsonSerializerOptions { PropertyNameCaseInsensitive = true }; //set the options here no case sensitive
            var result = JsonSerializer.Deserialize<List<T>>(json, opts); //déserialize the result to be used as a list of T
